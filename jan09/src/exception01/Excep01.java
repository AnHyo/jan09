 ...
 * 물론 특정 API는 컴파일시 예외를 처리하지 않느면 컴파일 되지 않는 예외도 있습니다.
 * 
 * -> 예외의 정의 : 실행시에 발생할 수 있는 모든 에러 상황
 * 
 * 자바에서는 예외가 발생하는 순서가 아래와 같습니다.
 * 
 * 컴파일 -> 실행 -> 실행중 예외 발생 -> VM이 발생된 예외의 종류 및 내용을 파악한 뒤
 * 예외 객체를 생성함 -> 발생된 코드를 밖으로 던짐(Throw) -> 예외의 콜 스택에 전이
 ...
 * Error의 하위 타입은 일반적으로 자바의 실행기(VM)에 관련된 에러 상황들을
 * 정의한 클래스 입니다.
 * 이러한 문제들은 프로그래머가 처리할 수 없는 것들로 VM, JRE의 전체 문제이기도 합니다.
 ...
 * Exception입니다.
 * 
 * Throwable클래스는 자식으로 Error을 가지고 있기 때문에 예외의 최고 클래스라고 하지
 * 않습니다.
 * 
 * 예외의 종류
 * 크게 두 가지로 나뉩니다.
...
public class Excep01 {
	public static void main(String[] args) {
		...
		//예외처리 형태
		try {
			System.in.read();//자바 안으로 들어올거다.
		} catch (IOException e) { //예외 발생시 알아서 처리하고 밑에꺼를 진행 할 수 있게 도와줌
			e.printStackTrace();
		} 
		...
		//catch가 받음 = 예외가 발생해서 처리
		// c:\temp\temp.txt 가 있었으면, 이걸 실행, 없어서 catch가 실행된 것임
